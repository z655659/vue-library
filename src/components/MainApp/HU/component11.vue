...
<template>
	...
	<div>
		<el-table :data="tabledata" border style="width: 100%">
			<el-table-column fixed prop="hu_id" label="HU-Id" width="150">
			</el-table-column>
			<el-table-column prop="unique_id" label="ユニークID" width="200">
			</el-table-column>
			<el-table-column prop="rel_requirement" label="関連基本要件" width="680">
			</el-table-column>	
...
//			})
		...
		methods: {
			...
			huo() {
				var self = this
				axios.get('http://192.168.10.13:5000/HUSummary/' + window.localStorage.getItem('b') + '/' + self.page_size + '/' + self.page_number)
					.then(res => {
						//debugger
						console.log(res.data.content, '这是获取页面1的列表')
						self.tabledata = res.data.content;
			...
			send(index){
              ...
              this.$router.push({name: 'HComponent6',params: {HU_id: this.data,ARL_id: this.data2,UN_id: this.data3,}});
     		},
        sendDEF(index){
              this.data = this.tabledata[index].hu_id
              this.$router.push({name: 'HDEFlist',params: {HU_id: this.data}});
        },
        newDEF(index){
              console.log(110110);
              this.data = this.tabledata[index].hu_id
              this.$router.push({name: 'HComponent4_new',params: {HU_id: this.data}});
        },
			handleSizeChange(val) {
				console.log(`每页 ${val} 条`);
				this.page_size = val;
				axios.get('http://192.168.10.13:5000/HUSummary/' + window.localStorage.getItem('b') + '/' + this.page_size + '/' + this.page_number)
					.then(res => {
