			...
			<label><h4 class="h4_form">H/U--</h4></label>
				<table class="table1" border="0" cellspacing="0" cellpadding="0">
					<tr class="single"><td class="single_th">担当者</td><td>
						<input type="text" v-model="content.author_name" @blur="modelshow($event)" @focus="focusStatus($event)"  class="ipt"/>
					...
					<tr class="double"><td class="double_th">トリガー</td><td>
						<input type="text" v-model="content.fp_trigger" @blur="modelshow($event)" @focus="focusStatus($event)"  class="ipt ipt_double"/>
						<p class="red">输入框不能为空</p>
					</td></tr>
					<tr class="single"><td class="single_th">動作</td><td>
						<input type="text" v-model="content.pf_action"  @blur="modelshow($event)" @focus="focusStatus($event)"  class="ipt"/>
						<p class="red">输入框不能为空</p>
					</td></tr>
					<tr class="double"><td class="double_th">シーケンス図</td><td>
						<input type="text" v-model="content.seq_diagram"  @blur="modelshow($event)" @focus="focusStatus($event)"  class="ipt"/>
						<p class="red">输入框不能为空</p>
					</td></tr>
					<tr class="single"><td class="single_th">アプリケーション</td><td>
						<input type="text" v-model="content.application" @blur="modelshow($event)" @focus="focusStatus($event)"  class="ipt ipt_double"/>
						<p class="red">输入框不能为空</p>
					</td></tr>
					<tr class="double"><td class="double_th">kernel</td><td>
						<input type="text" v-model="content.kernel" @blur="modelshow($event)" @focus="focusStatus($event)"  class="ipt"/>
						<p class="red" >输入框不能为空</p>
					</td></tr>
					<tr class="single"><td class="single_th">systemd</td><td>
						<input type="text" v-model="content.systemd" @blur="modelshow($event)" @focus="focusStatus($event)"  class="ipt ipt_double"/>
						<p class="red">输入框不能为空</p>
					</td></tr>
					<tr class="double"><td class="double_th">補足参照仕様書</td><td>
						<input type="text" v-model="content.supple_spec" @blur="modelshow($event)" @focus="focusStatus($event)" class="ipt"/>
						<p class="red">输入框不能为空</p>
					</td></tr>
					...
					<tr class="double"><td class="double_th">未検証</td><td>
						<input type="text" v-model="content.uncheck" @blur="modelshow($event)" @focus="focusStatus($event)" class="ipt"/>
						<p class="red" >输入框不能为空</p>
					</td></tr>
					<tr class="single"><td class="single_th">備考</td><td>
						<input type="text" v-model="content.remark" @blur="modelshow($event)" @focus="focusStatus($event)" class="ipt ipt_double"/>
						<p class="red">输入框不能为空</p>
					</td></tr>
				</table>
		</form>
	...
	export default {
			...
			methods:{
				...
				submitForm(event) {
			      ...
			      axios.post("http://192.168.10.13:5000/AnalysisContent",{content:this.content}).then((res) => {
...
//			   		axios.get("http://192.168.10.13:5000/AnalysisDelete/"+this.content.definition_id).then((res) => {
//			   			//alert("del成功")
//		       			})
//		       			.catch(function(err){
//				  	});
